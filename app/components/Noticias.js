...
function CardNoticia({ noticia }) {
  return (
    <a href='#' className='relative overflow-hidden rounded-xl flex items-end cursor-pointer group flex-shrink-0'
      style={{ backgroundImage: 'url(' + noticia.img + ')', backgroundSize: 'cover', backgroundPosition: 'center', width: '200px', height: '200px' }}>
      <div className='absolute inset-0 bg-black/50 group-hover:bg-black/60 transition' />
      <div className='relative z-10 p-3'>
        <h3 className={lexend.className} style={{ fontSize: '13px', color: 'white', fontWeight: '500', letterSpacing: '-0.03em', lineHeight: '1.3' }}>{noticia.titulo}</h3>
      </div>
    </a>
  )
}
          ...
          {todas.map(function(n) {
            return (
              <a key={n.id} href='#' className='relative overflow-hidden rounded-xl flex items-end cursor-pointer flex-shrink-0'
                style={{ backgroundImage: 'url(' + n.img + ')', backgroundSize: 'cover', backgroundPosition: 'center', width: '160px', height: '160px', scrollSnapAlign: 'start' }}>
                <div className='absolute inset-0 bg-black/50' />
                <div className='relative z-10 p-3'>
                  <h3 className={lexend.className} style={{ fontSize: '12px', color: 'white', fontWeight: '500', letterSpacing: '-0.03em', lineHeight: '1.3' }}>{n.titulo}</h3>
                </div>
              </a>
            )
          })}
