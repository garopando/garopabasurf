...
export default function EditarPost() {
  ...
  useEffect(function() {
    ...
    if (!id || !editor) return
    supabase.from('posts').select('*').eq('id', id).single().then(function({ data }) {
      if (data) {
        ...
        setDataPost(new Date(data.criado_em).toISOString().slice(0, 16))
  ...
  async function salvar() {
    ...
    if (error) {
      ...
      setTimeout(function() { setMensagem('') }, 3000)
    }
  }
  ...
  if (carregando) return <div style={{ padding: '100px', textAlign: 'center', color: '#9ca3af' }}>Carregando...</div>

  return (
    <div style={{ minHeight: '100vh', background: '#f9fafb' }}>
      <div style={{ background: 'black', padding: '16px 32px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <h1 className={lexend.className} style={{ color: 'white', fontSize: '20px', letterSpacing: '-0.06em' }}>Editar Post</h1>
        <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
          ...
          <button onClick={function() { window.open('/noticias/' + slugify(titulo), '_blank') }}
            style={{ background: 'transparent', color: '#60a5fa', border: '1px solid #1e40af', borderRadius: '8px', padding: '8px 16px', fontSize: '13px', cursor: 'pointer' }}>
            Ver post
          </button>
      ...
      <div style={{ maxWidth: '900px', margin: '0 auto', padding: '32px 16px' }}>
        {mensagem && (
          <div style={{ background: mensagem.includes('Erro') ? '#fef2f2' : '#f0fdf4', color: mensagem.includes('Erro') ? '#dc2626' : '#166534', padding: '12px 16px', borderRadius: '12px', marginBottom: '16px', fontSize: '14px' }}>
            {mensagem}
          </div>
        )}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '16px', marginBottom: '24px' }}>
          <div>
            ...
            <input type='text' placeholder='#surf #garopaba' value={categoria} onChange={function(e) { setCategoria(e.target.value) }}
