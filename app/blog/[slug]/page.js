...
export default function PostPage() {
  ...
  function processContent(html) {
    if (!html) return ''
    // YouTube -> embed (trata <a href="...">...</a> e URL solta)
    function ytEmbed(id) {
      return '<div class="yt-embed"><iframe src="https://www.youtube.com/embed/' + id + '" title="YouTube" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe></div>'
    }
    // primeiro: links em <a href="...youtube...">...</a>
    html = html.replace(/<a[^>]*href="https?:\/\/(?:www\.)?(?:youtube\.com\/watch\?v=|youtu\.be\/)([A-Za-z0-9_-]{11})[^"]*"[^>]*>.*?<\/a>/g, function(m, id) { return ytEmbed(id) })
    // depois: URLs soltas (que nao estao dentro de href)
    html = html.replace(/(?:^|\s)(https?:\/\/(?:www\.)?(?:youtube\.com\/watch\?v=|youtu\.be\/)([A-Za-z0-9_-]{11})[^\s<]*)/g, function(m, url, id) { return ' ' + ytEmbed(id) })
    return html.replace(
  ...
  return (
    <div className='min-h-screen bg-white'>
      <Navbar />
      {post.thumbnail && (
        <img src={post.thumbnail} alt={post.titulo} style={{ width: '100%', height: '450px', minHeight: '450px', maxHeight: '450px', objectFit: 'cover', display: 'block', marginTop: '64px' }} />
      )}
      <div style={{ maxWidth: '760px', margin: '0 auto', padding: '40px 16px 60px' }}>
        <Link href='/blog' style={{ color: '#9ca3af', fontSize: '13px', textDecoration: 'none', display: 'block', marginBottom: '24px' }}>← Voltar para o Blog</Link>
        {post.categoria && (
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '32px', paddingBottom: '32px', borderBottom: '1px solid #f3f4f6' }}>
          ...
          <a href={'https://www.facebook.com/sharer/sharer.php?u=' + encodeURIComponent('https://garopabasurf.app/blog/' + post.slug)}
          ...
          <a href={'https://wa.me/?text=' + encodeURIComponent(post.titulo + ' ' + 'https://garopabasurf.app/blog/' + post.slug)}
            target='_blank' rel='noreferrer'
          ...
          <a href={'https://twitter.com/intent/tweet?text=' + encodeURIComponent(post.titulo) + '&url=' + encodeURIComponent('https://garopabasurf.app/blog/' + post.slug)}
            target='_blank' rel='noreferrer'
          ...
          <button
            onClick={function() { navigator.clipboard.writeText('https://garopabasurf.app/blog/' + post.slug); alert('Link copiado!') }}
          ...
          <div style={{ marginTop: '60px', paddingTop: '40px', borderTop: '1px solid #f3f4f6' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px' }}>
              {recomendados.map(function(rec) {
                return (
                  <Link key={rec.id} href={'/blog/' + rec.slug} style={{ textDecoration: 'none' }}>
                    {rec.thumbnail && (
      ...
      <style>{`
        ...
        .post-content .yt-embed { position: relative; width: 100%; padding-bottom: 56.25%; margin: 24px 0; border-radius: 12px; overflow: hidden; }
        .post-content .yt-embed iframe { position: absolute; top: 0; left: 0; width: 100%; height: 100%; border: 0; }
        .post-content .yt-embed { position: relative; width: 100%; padding-bottom: 56.25%; margin: 24px 0; border-radius: 12px; overflow: hidden; }
        .post-content .yt-embed iframe { position: absolute; top: 0; left: 0; width: 100%; height: 100%; border: 0; }
