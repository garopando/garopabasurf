  ...
  return (
    <div className='min-h-screen bg-white'>
      ...
      <div style={{ maxWidth: '900px', margin: '0 auto', padding: '100px 16px 60px' }}>
        <h1 className={lexend.className} style={{ fontSize: '40px', fontWeight: '700', letterSpacing: '-0.06em', color: 'black', WebkitTextStroke: '0.5px black', marginBottom: '8px' }}>Notícias</h1>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
          {posts.map(function(post) {
            return (
              <a key={post.id} href={'/noticias/' + post.slug} style={{ textDecoration: 'none', display: 'flex', gap: '20px', borderBottom: '1px solid #f3f4f6', paddingBottom: '24px' }}>
                {post.thumbnail && (
